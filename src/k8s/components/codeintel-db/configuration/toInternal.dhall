...
let Simple/Codeintel-db = ../../../../simple/postgres/package.dhall
...
let PersistentVolumeClaim/toInternal
    ...
    = λ(cg : Configuration/Global.Type) →
        ...
        let c = cg.codeintel-db.PersistentVolumeClaim
...
let ConfigMap/toInternal
    ...
    = λ(cg : Configuration/Global.Type) →
        ...
        let c = cg.codeintel-db.ConfigMap

        in  { namespace, data.`codeintel-db.conf` = c.data.`codeintel-db.conf` }
...
let dataDiskVolumeMount =
      ...
      , mountPath = Simple/Codeintel-db.Containers.pgsql.volumes.data
      , name = "disk"
      }
...
let Containers/init/toInternal
    ...
    = λ(cg : Configuration/Global.Type) →
        let c =
              cg.codeintel-db.Deployment.Containers.correct-container-dir-permissions
...
let Containers/pgsql/toInternal
    ...
    = λ(cg : Configuration/Global.Type) →
        let c = cg.codeintel-db.Deployment.Containers.pgsql
...
let Test/Container/pgsql/Resources/none =
      ...
      :   ( Containers/pgsql/toInternal
              ( Fixtures.Config.Default
                with codeintel-db.Deployment.Containers.pgsql.resources
...
let Containers/pgsql-exporter/toInternal
    ...
    = λ(cg : Configuration/Global.Type) →
        let c = cg.codeintel-db.Deployment.Containers.pgsql-exporter
...
let Test/Container/pgsql-exporter/resources/none =
      ...
      :   ( Containers/pgsql-exporter/toInternal
              ( Fixtures.Config.Default
                with codeintel-db.Deployment.Containers.pgsql-exporter.resources
                     = Fixtures.Resources.EmptyInput
              )
          ).resources
        ≡ Fixtures.Resources.EmptyExpected
...
let Deployment/toInternal
    ...
    = λ(cg : Configuration/Global.Type) →
        ...
        let opts = cg.codeintel-db

        let Volumes =
              { disk.persistentVolumeClaim.claimName
                = opts.PersistentVolumeClaim.name
              }
        ...
        in  { namespace
            ...
            , Volumes
            , podSecurityContext
            , additionalVolumes = opts.Deployment.additionalVolumes
            , sideCars = cg.codeintel-db.Deployment.additionalSideCars
            }
