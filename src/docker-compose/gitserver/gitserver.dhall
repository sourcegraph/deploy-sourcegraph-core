...
let Simple/Gitserver = ../../simple/gitserver/package.dhall
...
let generate
    ...
    = λ(c : configuration.Type) →
        let name = "gitserver-0"

        let environment = Some (environment/toList c.environment)

        let simple = Simple/Gitserver.Containers.gitserver

        let mountDir = simple.volumes.repos
